...
public class AESFeistel
{
    ...
    /******************************************** 
        AESFeistel.cs private functions:
           - AddRoundKey
           - decryptBloc
           - deletePadding
           - encryptBloc
    ...
    ********************************************/
    ...
    private static byte[] encryptBloc(byte[] ina, byte[] key_array) 
    {
        byte[] tmp = new byte[ina.Length];
        ...
        {
            state = AESFeistel.SubBytes(state);
            state = AESFeistel.ShiftRows(state);
            state = AESFeistel.MixColumns(state);
        ...
        //FEISTEL CIPHER
        tmp1 = Feistel.encrypt(tmp1, key_array);
        tmp2 = Feistel.encrypt(tmp2, key_array);
        ...
        //Rounds 6 to 9
        for (int round = 6; round <Nr; round++) 
        {
            state = AESFeistel.SubBytes(state);
            state = AESFeistel.ShiftRows(state);
            state = AESFeistel.MixColumns(state);
    ...
    private static byte[] SubWord(byte[] ina) 
    {
        byte[] tmp = new byte[ina.Length];
        
        for (int i = 0; i < tmp.Length; i++)
            tmp[i] = (byte) (sbox[ina[i] & 0x000000ff] & 0xff);
    ...
    private static int[] inv_sbox = new int[]{ 
